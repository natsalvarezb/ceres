    ...
    try:
		...
		#ra = hd['RA']
	    	...
	    	sp,ra,dec,known_coords = GLOBALutils.simbad_coords(obname,mjd)
                ...
                print '\t\tSimbad coord retrieval failed'
		#ra,dec = -999,-999
                print '\t\tTrying to get coords from obname'
	        ra2,dec2 = GLOBALutils.getcoords(obname,mjd,filen=reffile)
	        if ra2 !=0 and dec2 != 0:
		    ra = ra2
		    dec = dec2
	        else:
                    print '\t\tObname coord retrieval failed'
                    ra = float(hd['RA'])
                    dec = float(hd['DEC'])
		    print '\t\tUsing coords found in the image header'
            ras,decs=ra,dec
